...
function RiskDrivers({ factors }: Props) {
	...
	return (
		<section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-shadow duration-300 hover:shadow-md sm:p-7">
			{/* Header */}
			<div className="flex items-start justify-between gap-4">
				<div>
					...
					<p className="mt-1 text-xs leading-5 text-slate-400">
						Space-weather parameters contributing to the current
						risk assessment.
					</p>
				</div>
				...
				{factors.length === 0 ? (
					...
					factors.map((factor) => {
						const score = Math.min(
							Math.max(factor.normalized_score, 0),
							100,
						);
						...
						return (
							<div
								key={factor.name}
								className="group rounded-2xl border border-slate-100 bg-slate-50/70 p-4 transition-all duration-200 hover:border-slate-200 hover:bg-white hover:shadow-sm"
							>
								{/* Driver heading */}
								<div className="flex items-center justify-between gap-3">
									<div className="flex min-w-0 items-center gap-3">
										<div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white text-base shadow-sm ring-1 ring-slate-100">
										...
										<div className="min-w-0">
											...
											<div className="mt-1 flex items-center gap-1.5">
												...
												<span
													className={`text-[10px] font-bold uppercase tracking-wide ${severity.text}`}
												>
													{severity.label}
												</span>
											</div>
									...
									<div
										className={`rounded-lg px-2.5 py-1 ${severity.bg}`}
									>
										<span
								...
								<div className="mt-4">
									<div className="h-2 overflow-hidden rounded-full bg-slate-200/70">
										<div
											className={`h-full rounded-full ${severity.bar} transition-all duration-1000 ease-out`}
											style={{
												width: `${score}%`,
											}}
										/>
			...
			{factors.length > 0 && (
				<div className="mt-5 flex items-start gap-2 border-t border-slate-100 pt-4">
					...
					<p className="text-[10px] leading-4 text-slate-400">
						Higher driver scores indicate stronger contribution
						to the current space-weather risk assessment.
					</p>
				</div>
			)}
		</section>
